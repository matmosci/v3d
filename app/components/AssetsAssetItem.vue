<template>
    <div
        ...
        <div class="h-32 w-full mt-1 rounded-md bg-black/30 overflow-hidden grid place-items-center">
            <img v-if="asset.thumbnail" :src="asset.thumbnail" alt="thumbnail"
        ...
        <div class="font-medium mx-1 mt-1 overflow-x-clip text-ellipsis">{{ asset.name }}</div>
        ...
        <div v-if="!showDeleteConfirm" class="flex items-center justify-between mx-1">
            <UButton icon="i-lucide-file-pen" variant="ghost" size="xs" class="text-blue-500" @click.stop="openAsset"
                title="Open">
            </UButton>
...
function openAsset() {
    if (!props.asset?._id || showDeleteConfirm.value) return;
    location.href = `/${props.asset._id}`;
}
...
function onDragStart(event) {
    // Keep drag type as `entity` to match existing drop handlers/editor contracts.
