...
export default class AssetLoader {
    constructor(ctx) {
        ...
        this.assetCompositionCache = new Map();
        ...
        this.ctx.events.on("object:ungroup", ({ id, recursive }) => {
            this.ungroupAssetInstance(id, !!recursive);
        });
    ...
    async load(id) {
        try {
            const resMeta = await fetch(`/api/assets/${id}`);
            this.metaData = await resMeta.json();
            const resInstances = await fetch(`/api/assets/${id}/instances`);
    ...
    getAssetsToLoadCount(instances = []) {
        const assetsSet = new Set(
            instances
                ...
                .filter((source) => source && source.sourceType === "file")
    ...
    async cacheAsset(asset) {
        ...
        try {
            const meta = await fetch(`/api/files/${asset}`).then((res) => res.ok ? res.json() : null);
        ...
        if (availableLevels.length <= 1) {
            ...
            const gltf = await loadGltf(`/api/files/${asset}/data?lod=${singleLevel}`);
        ...
        for (const level of availableLevels) {
            try {
                const gltf = await loadGltf(`/api/files/${asset}/data?lod=${level}`);
                const object = gltf.scene;
        ...
        if (successCount === 0) {
            const gltf = await loadGltf(`/api/files/${asset}/data?lod=0`);
            const object = gltf.scene;
            object.name = asset;
            this.ctx.assets.set(asset, object);
            return;
    ...
    async resolvePlacementObject(source, visited = new Set()) {
        ...
        if (source.sourceType === "asset") {
            return await this.buildAssetObject(source.sourceId, visited);
        }
    ...
    normalizeInstanceSource(payload = {}) {
        const sourceType = typeof payload.sourceType === "string" ? payload.sourceType : "file";
    ...
    async placeInstance(instance, transform) {
        ...
        if (source?.sourceType === "asset" && this.ctx?.assetId) {
            visited.add(this.ctx.assetId);
        }
    ...
    async captureAssetThumbnail() {
        if (!this.ctx.assetId || !this.ctx.renderer) return;
        ...
        try {
            ...
            await $fetch(`/api/assets/${this.ctx.assetId}/thumbnail`, {
                method: "POST",
                body: { thumbnail }
            });
    ...
    async confirmUserObjectPlacement(sourcePayload, transform) {
        ...
        const { handleInstancePlacement } = useAssetOwnership();
    ...
    async buildAssetObject(assetId, visited = new Set()) {
        if (!assetId) return null;
        
        // Check for cycles BEFORE checking cache
        if (visited.has(assetId)) {
            return createAssetCyclePlaceholder(assetId);
        }
        ...
        const cached = this.assetCompositionCache.get(assetId);
        ...
        try {
            const [metaRes, instancesRes] = await Promise.all([
                ...
                fetch(`/api/assets/${assetId}/instances`),
            ]);
            ...
            if (visited.size === 0) {
                this.assetCompositionCache.set(assetId, group);
            }
            ...
            console.error(`Failed to load asset contents ${assetId}`, error);
            return createAssetCyclePlaceholder(assetId);
        }
    }
    ...
    async ungroupAssetInstance(id, recursive = false) {
        if (!id) return;
        const object = this.findInstanceObjectById(id);
        if (!object) return;
        if (object.instanceSourceType !== "asset") return;

        const assetId = object.instanceSourceId;
        if (!assetId) return;

        const parentMatrix = this.composeMatrixFromTransform(this.getObjectTransform(object));
        const visited = new Set();
        if (this.ctx?.assetId) visited.add(this.ctx.assetId);

        const placements = await this.expandAssetInstances(assetId, parentMatrix, { recursive }, visited);
    ...
    async expandAssetInstances(assetId, parentMatrix, options = {}, visited = new Set()) {
        if (!assetId) return [];
        ...
        try {
            const instancesRes = await fetch(`/api/assets/${assetId}/instances`);
            ...
            for (const instance of instances) {
                ...
                if (source.sourceType === "asset" && options.recursive) {
                    const nested = await this.expandAssetInstances(
                        source.sourceId,
                        combinedMatrix,
                        options,
                        new Set(nextVisited)
            ...
            console.error(`Failed to ungroup asset ${assetId}`, error);
            return [];
        }
    }
    ...
    async createInstanceFromPayload(payload) {
        ...
        const res = await fetch(`/api/assets/${this.ctx.assetId}/instances`, {
            method: "POST",
    ...
    async deleteUserObjectInstance(id) {
        ...
        const { isOwner } = useAssetOwnership();
        
        if (isOwner.value) {
            // User owns the asset, delete from server
            const res = await fetch(`/api/assets/${this.ctx.assetId}/instances/${id}`, {
                method: "DELETE",
            });
    ...
    async confirmUserObjectTransform(id, transform) {
        ...
        if (isOwner.value) {
            ...
            // User doesn't own asset - apply transform locally only (temporary)
            const object = this.findInstanceObjectById(id);
            if (object) {
...
function createAssetCyclePlaceholder(assetId) {
    const group = new Group();
    group.name = assetId;
    ...
    placeholder.userData.isAssetPlaceholder = true;
